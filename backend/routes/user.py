...
def add_details(current_user=None):
    try:
        ...
        print(data)
        new_contact = ContactDetails(id=data['rollno'], college_email=data['email1'],
                                     personal_email=data['email2'], phone_number=data['phone'], home_town=data['home'])

        dob = datetime.strptime(data['dob'], '%Y-%m-%d')
        new_user_details = UserDetails(id=data['rollno'], dob=dob, type_of_degree=data['degree_type'],
                                       year_of_joining=data['join_year'], branch=data['branch'], nick_name=data['nick_name'])
        ...
        if 'profile' in data:
            photo = Media(sacc_id=data['rollno'],
                          path=data['profile'], media_type=1)
