...
def get_profile(current_user=None):
    try:
        if current_user:
            ...
            profile_photo = (
                Media.query.filter_by(id=current_user["roll_no"], media_type=1)
            ...
            profile_page = {
                ...
                "email1": contact_details.college_email
                if contact_details.college_email
                else "",
                "email2": contact_details.personal_email
                if contact_details.personal_email
                else "",
                ...
                if user_details.type_of_degree
                else "",
                "join_year": user_details.year_of_joining
                if user_details.year_of_joining
                else "",
    ...
    try:
        ...
        if not user_details:
            user_details = UserDetails(
                id=data["rollno"],
                dob=dob,
                type_of_degree=data["degree_type"],
                year_of_joining=data["join_year"],
                branch=data["branch"],
            ...
            user_details.nick_name = data.get("nick_name", user_details.nick_name)
            user_details.home_town = data.get("hometown", user_details.home_town)
...
def display_user_info():
    try:
        ...
        media_details = (
            Media.query.filter_by(sacc_id=user_id)
            .filter(Media.media_type.in_([1, 2]))
            .all()
        )
